...
- **可识别性与 4 份来源**：4 份来源时若 ψ、α 的二次谐波方向独立（4 个谐波
  系数满秩），直接拟合 4 个谐波（不估轴对称截距）并正常求解；≥5 份且截距
  独立可估时再拟合截距。只有真正不可识别的角度组合才报缺口
- **一致性拒绝（422）**：口径、曲率半径或遮罩分区（环带边界/等效半径）
  不一致的版本不能进入同一研究；同一冻结版本不可重复引用；缺少采样时间
  的来源被 Pydantic 拒绝
- **欠秩缺口**：刀口方位固定只转镜面时架位项被截距吸收（反之亦然），或
  ψ 与 α 同步共线变化使两对象散混淆时，拟合矩阵秩不足；此时**不强行给伪
  结果**，逐分量指出需补的角度（如“方位相差非 0°/90°”“ψ 与 α 需独立
  变化或补第 5 份来源”）；创建时档案仍保留，补角度后重解
- **排除与留一**：调用方注明原因排除单次来源（排除后仍需 ≥4 份），留一法
  逐份剔除重解，按（主轴偏移 ≤ 阈值、幅值落在全解置信区间、可疑区段不变）
  汇总 `stable`；留一不修改研究数据。**留一与定稿严格使用所选版本冻结的
  来源与排除快照**（从该版本结果重建），不读取研究当前状态——排除/追加后
  查询历史版本的留一报告或定稿历史版本，不会混入当前来源
