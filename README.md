...
角度约定（从光学面观察、0° 参考方向、逆时针为正、单位度）与拟合参数
（信号字段 `la_residual`/`la_error`、置信水平、稳定性阈值）在研究级冻结。

- **一致性拒绝（422）**：口径、曲率半径或遮罩分区（环带边界/等效半径）
  不一致的版本不能进入同一研究；同一冻结版本不可重复引用
- **欠秩缺口**：刀口方位固定只转镜面时架位项被截距吸收（反之亦然），拟合
  矩阵秩不足时**不强行给伪结果**，逐分量指出需补的角度（如“方位相差非
  0°/90°”）；创建时档案仍保留，补角度后重解
